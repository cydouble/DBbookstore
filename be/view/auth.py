...
# from be.model import about_token
...
def register_page():
  ...
  if request.method == "POST":
    ...
    code,msg = regis.register(user_info.get('username'),user_info.get('password'))
...
def unregister_page():
  ...
  if request.method == "POST":
    user_info = request.values.to_dict()
    regis = user.register_action()
    code,msg = regis.unregister(user_info.get('username'),user_info.get('password'))
    return jsonify({"message":msg}),code
...
def login():
  ...
  if request.method == "POST":   # POST请求
    ...
    code,msg = logi.login(user_info.get('username'),user_info.get('password'),user_info.get('terminal'))
    global token
    if code == 200:
      token = about_token.generate_auth_token(user_info.get('username'),user_info.get('terminal'))
    else:
      token = ''
...
def change_pwd():
  ...
  if request.method == "POST":   # POST请求
    ...
    code,msg = old2new.change_password(user_info.get('username'),user_info.get('old_pwd'),user_info.get('new_pwd'))
    return jsonify({'message':msg}),code
